...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		int[][] doubleMatrix = new int[adjMatrix.length][adjMatrix.length];
		for (int row = 0; row<adjMatrix.length; row++) {
			for (int col = 0; col<adjMatrix.length; col++) {
				doubleMatrix[row][col] = multiplyMatricesCell(adjMatrix, adjMatrix, row, col);
			}
		}
		for (int i = 0; i<doubleMatrix.length; i++) {
			while (doubleMatrix[v][i] > 0) {
				res.add(i);
				doubleMatrix[v][i] -= 1;
			}
		}
	...
	private int multiplyMatricesCell (int[][] m1, int[][] m2, int row, int col) {
		int cell = 0;
