...
class MapNode implements Comparable<MapNode>
{
	...
	void setDistanceToGoal(MapNode node) {
		distanceToGoal = this.location.distance(node.getLocation());
	}
	...
	double getDistanceToGoal(MapNode node) {
		return this.location.distance(node.getLocation());
	}
	
	@Override
	public int compareTo (MapNode n) {
		return Double.compare(this.distanceToStart + this.distanceToGoal, n.getDistanceToStart()+ n.getDistanceToGoal());
	}
